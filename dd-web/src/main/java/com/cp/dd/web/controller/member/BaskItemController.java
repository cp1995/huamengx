...
import com.cp.dd.common.vo.sport.BaskItemVO;
...
@AllArgsConstructor
@RestController
@RequestMapping("/api/baskItem")
@Api(value = "/item", tags = "篮球录入接口")
@Slf4j
public class BaskItemController {

    private IBaskItemService baskItemService;


    @AddOperLog(name = "新增篮球报告信息")
    @PostMapping("/save")
    @ApiOperation(value = "新增篮球报告信息", notes = "新增篮球报告信息")
    public Result save(@Valid BaskItemForm baskItemForm) {
        baskItemService.save(baskItemForm);
        return Result.success();
    }


    @AddOperLog(name = "修改篮球报告信息")
    @PostMapping("/update")
    @ApiOperation(value = "修改篮球报告信息", notes = "修改篮球报告信息")
    public Result update(@Valid BaskItemForm baskItemForm) {
        baskItemService.update(baskItemForm);
        return Result.success();
    }
    @GetMapping("/getPage")
    @ApiOperation(value = "报告分页列表", notes = "用户分页列表")
    public Result<PageModel<BaskItemVO>> getPage(@Valid PageQuery pageQuery,
        ...
        return Result.success(baskItemService.getPage(pageQuery,childName,phone,name,createBy,parentName,areaId));
    }

    @GetMapping("/getDataPage")
    @ApiOperation(value = "数据报告分页列表", notes = "数据报告分页列表")
    public Result<PageModel<BaskItemVO>> getDataPage(@Valid PageQuery pageQuery,
                                             @ApiParam("小朋友姓名") @RequestParam(required = false) String childName,
                                             @ApiParam("家长手机号") @RequestParam(required = false) String phone,
                                             @ApiParam("场次名称") @RequestParam(required = false) String name,
                                             @ApiParam("创建人") @RequestParam(required = false) String createBy,
        ...
        return Result.success(baskItemService.getDataPage(pageQuery,childName,phone,name,createBy,areaId,sort,ascOrDesc));
    }
    ...
    public void export(HttpServletResponse response, HttpServletRequest request,
        ...
        baskItemService.export(response,request,childName,phone,name,createBy,areaId,sort,ascOrDesc);
    }
    ...
    public Result<?> importWord(@RequestParam("file") MultipartFile file) {
        baskItemService.importItem(file);
        return Result.success();
    }
    @AddOperLog(name = "公众号获取报告")
    @GetMapping("/getData")
    @ApiOperation(value = "公众号获取报告", notes = "公众号获取报告")
    public Result<List<BaskItemVO> > getData(
                                                 @ApiParam("小朋友姓名") @RequestParam(required = false) String childName,
                                                 @ApiParam("家长手机号") @RequestParam(required = false) String phone,
    ...
    public Result<BaskItemVO> detail(
            @ApiParam("id") @RequestParam(required = true) Long id

    ) {
        return Result.success(baskItemService.detail(id));
    }

    @GetMapping("/del")
    @AddOperLog(name = "删除篮球报告")
    @ApiOperation(value = "删除篮球报告", notes = "删除篮球报告")
    public Result del(
            @ApiParam("id") @RequestParam(required = true) Long id) {
        baskItemService.del(id);
        return Result.success();
    }
          ...
          try {
              HttpServletRequest request = ((ServletRequestAttributes) RequestContextHolder.getRequestAttributes()).getRequest();
                  String path = request.getSession().getServletContext().getRealPath("/");
                   path = path + File.separator + "篮球数据模板-格式统一为文本格式.xlsx";
                   // path是指欲下载的文件的路径。
