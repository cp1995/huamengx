package com.cp.dd.schedule.model;

import com.cp.dd.schedule.util.CronUtil;
import lombok.Data;
...
public class TaskBeanManager {
    ...
    public static void setTaskMap(Map<String, ITask> taskMap) {
        taskMap.forEach((instanceName, task) -> {
            ...
            for (Method method : methods) {
                ...
                if(scheduled == null && taskAnnotation == null){
                    continue;
                }
                ...
                if (taskAnnotation != null) {
                    taskBean.setDescription(StringUtils.isBlank(taskAnnotation.desc()) ? taskAnnotation.desc() : taskAnnotation.value());
    ...
    public static void setNextExecuteTime(TaskBean taskBean) {
        if (StringUtils.isNotBlank(taskBean.getCron())) {
            taskBean.setNextExecuteTime(CronUtil.getNextExecution2(taskBean.getCron()));
            return;
        }
